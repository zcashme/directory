...
| `/status-app` | status.zcash.me | Service health dashboard (polls /api/health) | [live] |
...
| `/api/lookup/[username]` | GET | Public profile lookup by username (no API key) | 300s |
| `/api/directory?q=&limit=&cursor=&verified_only=` | GET | Profile search with multi-tier ranking, pagination, CORS | 30s |
| `/api/social?platform=&handle=` | GET | Social handle -> Zcash address lookup (11 platforms) | 300s |
| `/api/health` | GET | Internal service health: directory, verifications, zcashme | 600s (route `revalidate`, public, no guard) |

Guarded routes (`resolve`, `lookup`, `directory`, `social`) use `enforceApiGuard()` for
authentication; `jsonResponse()` applies `s-maxage` CDN caching internally. `/api/health`
is public and uses Next.js `revalidate` instead.
