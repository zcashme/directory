 ...
 * Accepts either a timestamp string/number or a profile object with last_verified_at/last_verified.
 */
export function getLastVerifiedLabel(input) {
  const ts = typeof input === 'object'
    ? (input?.last_verified_at || input?.last_verified)
    : input;
  if (!ts) return "n/a";
