...
export async function oneclickTokens() {
  ...
  if (!r.ok) return { error: "Failed to load tokens from API" };
...
export async function oneclickQuote(payload) {
  ...
  const text = await r.text();
  if (!r.ok) return { error: "Could not get quote. Check your input and try again." };
...
export async function oneclickStatus(params) {
  ...
  if (!r.ok) return { error: "Could not check swap status. Try again in a moment." };
